...
function RetentionHeatmap({ matrix, rowLabels, colLabels }: any) {
  ...
  return (
    <div className="overflow-auto">
      <table className="text-xs border-collapse">
        <thead>
          <tr>
            <th className="px-3 py-2 text-left text-white/50">Cohort</th>
            {colLabels.map((cl: string, ci: number) => (
              <th key={ci} className="px-2 py-2 text-center text-white/50 min-w-[60px]">P{ci}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {matrix.map((row: any[], ri: number) => (
            <tr key={ri}>
...
export function SegmentsView({ projectId }: Props) {
  ...
  async function loadColumns() {
    ...
    setLoadingCols(true);
    try {
      const data = await getCohortColumns(projectId) as any;
      setAllCols(data.all_columns || []);
      setNumCols(data.numeric_columns || []);
      setDtCols(data.datetime_columns || []);
      setColsLoaded(true);
    } catch (e) {
  ...
  async function handleRfm() {
    ...
    try {
      ...
      setRfmResult(data);
    } catch (e: any) { setError(e.message || "RFM failed."); }
  ...
  async function handleRetention() {
    ...
    } catch (e: any) { setError(e.message || "Retention failed."); }
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
              ...
              { label: "Date Column", val: dateCol, set: setDateCol, opts: [...dtCols, ...allCols.filter(c => !dtCols.includes(c))] },
              { label: "Revenue Column", val: revenueCol, set: setRevenueCol, opts: numCols },
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="rounded-lg bg-white/[0.03] border border-white/[0.07] p-4 text-center">
                  <div className="text-2xl font-bold text-indigo-400">{rfmResult.total_customers?.toLocaleString()}</div>
                  <div className="text-xs text-white/50 mt-1">Total Customers</div>
                </div>
                <div className="rounded-lg bg-white/[0.03] border border-white/[0.07] p-4 text-center">
                  <div className="text-2xl font-bold text-indigo-400">${rfmResult.total_revenue?.toLocaleString()}</div>
                ...
                {Object.entries(rfmResult.segment_counts || {}).map(([seg, count]) => (
                  <div key={seg} className="rounded-lg bg-white/[0.03] border border-white/[0.07] p-3">
                    <div className="flex items-center gap-2 mb-1">
                      <div className="w-2 h-2 rounded-full" style={{ background: SEGMENT_COLORS[seg] || "#94a3b8" }} />
                      <span className="text-xs text-white/60 font-medium">{seg}</span>
                    </div>
                    <div className="text-xl font-bold text-white">{String(count)}</div>
                    {rfmResult.segment_stats?.[seg] && (
                      <div className="text-xs text-white/40 mt-1">
                        Avg ${rfmResult.segment_stats[seg].avg_monetary?.toFixed(0)}
                      </div>
                    )}
                  </div>
                ))}
                  ...
                  <thead className="sticky top-0 bg-[#0f172a]">
                    <tr>
                      {["Customer", "Recency (days)", "Frequency", "Monetary", "R", "F", "M", "Segment"].map((h) => (
                        <th key={h} className="px-3 py-2 text-left text-white/50 font-medium border-b border-white/[0.07]">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {(rfmResult.customers || []).slice(0, 50).map((c: any, i: number) => (
                      <tr key={i} className="border-b border-white/[0.04]">
                        ...
                        <td className="px-3 py-2 text-white/70">${c.monetary?.toFixed(2)}</td>
                        ...
                        <td className="px-3 py-2">
                          <span className="px-2 py-0.5 rounded-full text-xs font-medium" style={{ background: `${SEGMENT_COLORS[c.segment]}22`, color: SEGMENT_COLORS[c.segment] || "#94a3b8" }}>
                            {c.segment}
                          </span>
                        </td>
                      </tr>
