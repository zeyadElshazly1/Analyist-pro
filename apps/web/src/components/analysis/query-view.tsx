...
// Monaco is browser-only — load lazily with no SSR
const MonacoEditor = dynamic(() => import("@monaco-editor/react").then((m) => m.Editor), {
  ssr: false,
  loading: () => (
    <div className="flex h-[160px] items-center justify-center bg-[#0d1117] text-xs text-white/30">
      Loading editor…
    </div>
  ),
});
...
export function QueryView({ projectId }: Props) {
  ...
  useEffect(() => {
    getQuerySchema(projectId)
      .then((data: any) => setSchema(data.columns || []))
      .catch(() => {});
  }, [projectId]);

  // Re-register autocomplete when schema changes
  useEffect(() => {
    if (!monacoRef.current || schema.length === 0) return;
    completionDisposable.current?.dispose();
    completionDisposable.current = monacoRef.current.languages.registerCompletionItemProvider("sql", {
      provideCompletionItems: () => ({
        suggestions: schema.map((col) => ({
          label: col.name,
          kind: monacoRef.current.languages.CompletionItemKind.Field,
          insertText: col.name,
          detail: col.dtype,
          documentation: `Column: ${col.name} (${col.dtype})`,
        })),
      }),
    });
  ...
  const handleRun = useCallback(async () => {
    const query = editorRef.current ? editorRef.current.getValue() : sql;
    if (!query.trim()) { setError("Enter a SQL query."); return; }
    setLoading(true); setError("");
    try {
      const data = await executeQuery(projectId, query) as any;
      setResult(data);
    } catch (e: any) {
  ...
  function insertColumnName(colName: string) {
    ...
    editor.executeEdits("insert-column", [{ range: sel, text: colName, forceMoveMarkers: true }]);
    editor.focus();
  }
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-white/50">
          Run SQL queries on your dataset. The table is available as{" "}
          <code className="text-indigo-400">data</code>.
        </p>
      </div>
        ...
        <div className="rounded-xl border border-white/[0.07] bg-white/[0.03] p-4">
          ...
          <div className="space-y-1 max-h-[300px] overflow-y-auto">
            {schema.map((col) => (
              <div
                key={col.name}
                className="group cursor-pointer rounded px-2 py-1 hover:bg-white/[0.05]"
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="rounded-xl border border-white/[0.07] overflow-hidden">
            <div className="flex items-center justify-between px-3 py-2 bg-white/[0.03] border-b border-white/[0.07]">
              <span className="text-xs text-white/40">SQL Editor</span>
              <span className="text-xs text-white/20">Ctrl+Enter to run · click column to insert</span>
            </div>
            <MonacoEditor
              height="160px"
              language="sql"
          ...
          <div className="flex gap-2">
            ...
            >
              {loading ? (
                ...
                <><Terminal className="h-4 w-4" />Run Query</>
              )}
            </Button>
                  ...
                  <thead className="sticky top-0 bg-[#0f172a]">
                    <tr>
                      {(result.columns || []).map((col: string) => (
                        <th
                          key={col}
                          className="px-3 py-2 text-left text-white/50 font-medium border-b border-white/[0.07] whitespace-nowrap"
                        >
                          {col}
                        </th>
                      ))}
                    ...
                    {(result.rows || []).map((row: any, ri: number) => (
                      <tr key={ri} className="border-b border-white/[0.04]">
                        {(result.columns || []).map((col: string) => (
                          <td
                            key={col}
                            className="px-3 py-2 text-white/70 whitespace-nowrap max-w-[200px] truncate"
                          >
                            {row[col] == null ? (
