...
|---|------|-------------|------|------|-----------|-------------|-----------------|-----------|-----------------|----------------|--------------|----------|----------------|---------------------|
| 1 | auto_insurance_data.xlsx | Auto insurance / risk | ~1k | ~20 | Excel | Upload, intake, health score, findings list, export all completed without error | Cleaning Review grade shown as raw Python object: `Grade {'score': 70, 'grade': 'B', 'label': 'Good'}` instead of formatted label | Yes | No | No | No | P2 | Format health/cleaning score object cleanly in Cleaning Review UI — display grade letter and label, not raw dict | Defer — schedule when a second file triggers the same display issue, or when confirmed reproducible across all xlsx uploads |
...
|----------|---------|-----------------|---------------------|
| 1 | #1 | Format health/cleaning grade object in Cleaning Review UI | Second file or confirmed reproducible on all xlsx |
